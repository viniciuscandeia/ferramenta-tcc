...
1. `SRS-completo.md` — documento de requisitos no padrão IREB §3.3.3 (ISO/IEC/IEEE 29148)
2. `SRS-completo-leigo.md` — versão em linguagem acessível para aprovação no Gate 3
3. `spec/*.feature` — cenários Gherkin para cada RF com modal `DEVE` (D20, D22)
4. `spec/_skipped.md` — registro de RFs não-cobertos por Gherkin (modal `DEVERIA`/`PODE`)
5. `tests/unit/` + `tests/acceptance/` — step definitions em estado RED para 3 frameworks (D20)
6. `TESTING-STRATEGY.md` — estratégia de teste por RNF (D21)
7. `README-TESTS.md` — instruções de execução para os 3 frameworks (D23)
...
3. Ler artefatos de entrada opcionais (se existirem):
   - `03.4-premissas.md`, `conflitos-detectados.md`
...
- Input: `03.1-funcionais.md` + `03.2-qualidade.md`
...
- Montar `SRS-completo.md` com as 6 seções IREB §3.3.3 completas
- Seção 3: RFs em EARS + RFC 2119 (da saída do Passo 1)
- Seção 4: RNFs com bucket + métrica + critério de aceite
- Seção 5: restrições + premissas + `glossario.md` integrado
...
- Input: `03.1-funcionais.md` com campo modal preenchido (da skill `priorizacao` M2)
- Filtro rígido (D22): gerar `.feature` apenas para RFs com modal `DEVE`
- Para cada RF `DEVE`: criar `spec/<id-rf>-<slug>.feature` com Feature + Scenarios (caminho feliz + até 2 bordas)
- Gerar `spec/_skipped.md` com todos os RFs de modal `DEVERIA`/`PODE`

**Passo 4 — step-defs-red**
- Invocar `core/skills/step-defs-red/SKILL.md`
- Input: `spec/*.feature` gerados no Passo 3
- Gerar step definitions em estado RED (falham ao executar — sem implementação) para:
  - **Pytest-BDD** → `tests/acceptance/test_<slug>.py`
  - **Cucumber-js** → `tests/acceptance/<slug>.steps.js`
  - **SpecFlow** → `tests/acceptance/<Slug>Steps.cs`
...
- Input: `03.2-qualidade.md` (todos os RNFs finalizados)
...
- Saída: `TESTING-STRATEGY.md`

**Passo 6 — readme-tests**
- Invocar `core/skills/readme-tests/SKILL.md`
- Input: `TESTING-STRATEGY.md` + estrutura `tests/`
- Gerar `README-TESTS.md` com 3 seções (uma por framework: Pytest-BDD, Cucumber-js, SpecFlow)
...
- Input: `SRS-completo.md` (normativo, gerado no Passo 2)
- Gerar `SRS-completo-leigo.md`: mesma estrutura, linguagem acessível ao stakeholder leigo
...
Ativado quando `checker` retorna `analyze-report.md` com ≥ 1 issue de severidade CRITICAL.

1. Ler `analyze-report.md` — identificar quais artefatos e quais IDs de requisito estão afetados
...
3. Executar **somente as skills correspondentes** aos artefatos afetados:
   ...
   - CRITICAL em TESTING-STRATEGY → re-executar Passo 5 (testing-strategy)
   - CRITICAL em README-TESTS → re-executar Passo 6 (readme-tests)
   - Sempre regenerar SRS-completo-leigo se SRS-completo for alterado (Passo 7)
...
|---|---|---|---|
| `SRS-completo.md` | Normativa | Sempre — Passo 2 | `checker`, equipe técnica |
| `SRS-completo-leigo.md` | Leigo | Sempre — Passo 7 | Gate 3 (aprovação leigo) |
| `spec/*.feature` | Única (técnica) | Por RF com modal DEVE — Passo 3 | `checker` M4, repositório |
| `spec/_skipped.md` | Única (técnica) | Sempre — Passo 3 | Transparência — RFs não-DEVE |
| `tests/unit/` + `tests/acceptance/` | Única (RED) | Sempre — Passo 4 | `checker` M4, dev team |
| `TESTING-STRATEGY.md` | Única (técnica) | Sempre — Passo 5 | `checker` M4 |
| `README-TESTS.md` | Única (técnica) | Sempre — Passo 6 | dev team |

---

## COMPATIBILIDADE DE PLATAFORMA

**Claude Code:** sub-agente isolado via `Task()`. Recebe `m3-srs-specs-tests.md` como contexto.
