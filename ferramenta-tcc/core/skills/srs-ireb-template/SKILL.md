...
2. **Zero omissão silenciosa.** Contagem de RFs na seção 3 == contagem em `03.1-funcionais.md`. Contagem de RNFs na seção 4 == contagem em `03.2-qualidade.md`. Divergência = erro de geração.
...
- NÃO executar sem `visao-produto-normativo.md`, `03.1-funcionais.md`, `03.2-qualidade.md`, `03.3-restricoes.md`, `glossario.md` — artefatos obrigatórios
...
**Seção 1 — Introdução:**
- 1.1 Objetivo: problema resolvido, usuários-alvo, contexto geral (de `visao-produto-normativo.md`)
...
- 2.2 Stakeholders: tabela (papel + interesse + influência) de `visao-produto-normativo.md`
- 2.3 Restrições de Design: `03.3-restricoes.md` subtipos legal/técnica/organizacional/temporal
- 2.4 Premissas: `03.4-premissas.md` com impacto se falsas; se ausente: "Nenhuma premissa formal registrada nesta fase"
- Se `conflitos-detectados.md` existir: nota em 2.3 indicando conflitos detectados + status

**Seção 3 — Requisitos Funcionais:**
Inserir tabela EARS de `requisito-ears`. Organizar por módulo/processo se `visao-produto-normativo.md` indicar agrupamentos naturais; caso contrário, manter ordem de IDs.
...
- 5.1 Interfaces: APIs de terceiros, sistemas legados, dispositivos físicos de `03.3-restricoes.md` ou `visao-produto-normativo.md`; se nenhum: "Interfaces externas não identificadas nesta fase — detalhar em fase de design"
- 5.2 Glossário: conteúdo completo de `glossario.md`

**Seção 6 — Matriz de Rastreabilidade:**
Tabela: Objetivo de negócio (M1) | RF/RNF | Spec (.feature) | Test
- Coluna spec: `spec/<id>.feature` para RFs DEVE; N/A para DEVERIA/PODE
...
- [ ] Contagem RFs na seção 3 == contagem em `03.1-funcionais.md`
- [ ] Contagem RNFs na seção 4 == contagem em `03.2-qualidade.md`
...
Salvar como `SRS-completo.md` (tamanho esperado: 300–600 linhas conforme projeto).
